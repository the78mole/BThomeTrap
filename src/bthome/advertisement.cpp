...
namespace bthome
{
    ...
    {
        uint16_t size = this->m_dataIdx + measurement.getPayloadSize();
        uint16_t maxsize = constants::BLE_ADVERT_MAX_LEN;
        if (size > maxsize)
        {
            ESP_LOGE(_ADVERT_LOG_NAME, "Unable to add sensor %d (len) > %d (max)", size, maxsize);
            return false;
        }
